...
Pcrr2_ss = (pi^2*E*Ir)./(Lr.^2);
Pcrs2_ff = (pi^2*E*Is)./((Ls/2).^2);
...
sigmacrr = zeros(1,length(Lr))+sigyield*Ar;

%Plot
figure(1)
hold on
grid on
plot(Ls,Pcrs2_ss,Ls,Pcrs2_ff,Ls,sigmacrs,'linewidth',2)
title('Force vs Beam Length of Square Hollow Beam')
...
hold on
grid on
plot(Lr,Pcrr2_ss,Lr,Pcrr2_ff,Lr,sigmacrr,'linewidth',2)
title('Force vs Beam Length of Rectangle Beam')
xlabel('Beam Length (in)')
ylabel('Force (lb)')
